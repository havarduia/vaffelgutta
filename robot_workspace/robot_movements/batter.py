from robot_workspace.assets.positions import tools, offsets
...
def place_cup_at_filling_station(bot: Wafflebot, is_holding_cup:bool = False):
    ...
    cam_pos                         = numphy.matrix(getattr(tools, "tool_camera"))

    front_of_filling_station_offset = numphy.matrix(getattr(offsets, "front_of_waffle_iron"))
...
def pick_up_cup_from_filling_station(bot: Wafflebot):
    ...
    front_of_filling_station_offset = numphy.matrix(getattr(offsets, "front_of_filling_station"))
...
def pour_batter(bot: Wafflebot):
    ...
    front_of_waffle_iron_offset = numphy.matrix(getattr(offsets, "front_of_waffle_iron"))
    pour_offsets = [
        numphy.matrix(getattr(offsets, "pour_a")),
        numphy.matrix(getattr(offsets, "pour_b")),
        numphy.matrix(getattr(offsets, "pour_c")),
        numphy.matrix(getattr(offsets, "pour_d"))
    ]
