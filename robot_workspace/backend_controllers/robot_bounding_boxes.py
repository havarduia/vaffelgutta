...
def _endpoints_from_bb(corners):
    ...
    for corner in corners:
        ...
        if corner[2] > maximum[1]: maximum[2] = corner[2]
    return(minimum, maximum)

def _rotate_cube_by_waist_angle(corners, waist_angle):
    for corner in corners:
        x = corner[0]
        y = corner[1]
        # z = corner[2]
        radius = numphy.hypot(x, y)
        theta_0 = numphy.arctan2(y, x)  
        corner[0] = float(radius * numphy.cos(theta_0 + waist_angle))
...
def _roll(corners, angle, origin):
    ...
    for corner in corners:
        ...
        #corner[0] = corner[0]
        corner[1] = y0+float(radius * numphy.cos(theta_0 + angle))
        corner[2] = z0+float(radius * numphy.sin(theta_0 + angle))
    return corners
...
def update_robot_bounding_box(
    ...
    shoulder_corners = _rotate_cube_by_waist_angle(shoulder_corners,waist_angle)
    ...
    for dt in range(arm_subdivisions): 
        ...
        subdivision_end[1] = arm_1_division_dimensions[1]/2 #the width (y) of arm1 is always 4cm
        #create subdivisions
    ...
    arm_link_origin[0] += 6e-2 # x should be nudged forward a wee to account for the length of the arm link         
    # arm_link_origin[1] +=0 # y coordinate already fixed 
    ...
    arm_link_offset[0] = -8e-2 # measured by hand
    ...
    bounding_boxes["arm_link"] = (arm_link_origin, arm_link_origin)
    ...
    "second arm joint"
    # Declare arm dimensions
    arm_2_origin = arm_link_origin
    arm_2_dimensions = [20e-2, 6e-2,3e-2] # measured by hand
    ...
    for dt in range(arm_subs): 
        ...
        sub_end[1] = arm_2_dimensions[1]/2 #the width (y) of arm2 is always 6cm
        #create subs
        sub_start[0] = arm_2_min[0]+arm_2_dimensions[0]*dt/DT     # start x
        sub_start[2] = arm_2_min[2]  # start z
        sub_end[0] = arm_2_min[0]+(arm_2_dimensions[0]*(dt+1)/DT)   # end x
        sub_end[2] = arm_2_min[2] + arm_2_dimensions[2] # arm 2 ends at an offset exual to its own size
        # create a cube and apply rotation
        ...
        sub_corners = _rotate_cube_by_waist_angle(sub_corners, waist_angle)
    ...
    motor_offset[0] = 19e-2 # Also measured by hand - the distance from the elbow joint to the beginning of the motor
    # Determine corners
    ...
    motor_corners = _roll(motor_corners, forearm_roll, motor_origin)
    motor_corners = _pitch(motor_corners,elbow_angle, arm_2_mount_origin)
    motor_corners = _pitch(motor_corners, shoulder_angle, shoulder_origin)
    motor_corners = _rotate_cube_by_waist_angle(motor_corners, waist_angle)
    (motor_min, motor_max) = _endpoints_from_bb(motor_corners)
    bounding_boxes["motor"] = (motor_min, motor_max)

    "wrist"
    # Declare arm dimensions
    wrist_origin = arm_link_origin
    wrist_origin[0] += 30e-2 # the distance between shoulder and wrist
    wrist_dimensions = [7e-2,6e-2,6e-2] # measured by hand, and using technical drawing on interbotix docs website for reference
    wrist_offset = [-elem/2 for elem in wrist_dimensions] # y, z offsets are half of the arm size
    ...
    wrist_corners = _pitch(wrist_corners,wrist_angle,wrist_origin)
    wrist_corners = _roll(wrist_corners, forearm_roll, wrist_origin)
    wrist_corners = _pitch(wrist_corners, elbow_angle, wrist_origin)
    wrist_corners = _pitch(wrist_corners, shoulder_angle, shoulder_origin)
    wrist_corners = _rotate_cube_by_waist_angle(wrist_corners, waist_angle)
    ...
    gripper_corners = _roll(gripper_corners,gripper_roll,gripper_origin)
    gripper_corners = _pitch(gripper_corners,wrist_angle, gripper_origin)
    gripper_corners = _roll(gripper_corners, forearm_roll, gripper_origin)
    gripper_corners = _pitch(gripper_corners,elbow_angle, gripper_origin)
    gripper_corners = _pitch(gripper_corners, shoulder_angle, shoulder_origin)
    gripper_corners = _rotate_cube_by_waist_angle(gripper_corners, waist_angle)
